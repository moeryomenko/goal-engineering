...
const patternsDest = path.join(PACKAGE_ROOT, 'patterns');
if (await exists(registrySrc)) {
  await rm(patternsDest, { recursive: true, force: true });
  await cp(path.join(REPO_ROOT, 'patterns'), patternsDest, { recursive: true });
  console.log('bundled patterns/ → tools/goal-init/patterns/');
}